...
- [Vehicles.meta](#vehiclesmeta)
  ...
  - [Extras](#extras)
    ...
    - [Preventing Specific Extras from Spawning](#preventing-specific-extras-from-spawning)
  - [Type](#type)
...
The **modelName** defines the .yft name within the stream folder. 
The **txdName** is the name of the texture dictionary that the vehicle uses. 
The **handlingID** links directly to the handling.meta name.
The **gamename** doesn't have a lot of use (that we know of) so is best to set it to the same as the modelname.
The **vehicleMakeName** is what company made the vehicle. E.G:
...
The **animConvRoofDictName** is set to what the .ycd file is called once it has been exported from zmodeler and put into the vehicles stream folder.
The **animConvRoofName** is the name of the animation within the .ycd file. Get this from the animation window within zmodeler.
...
The type flag defines what type of vehicle it is. This works in conjunction with the [txdRelationships](#txd-relationships). You can set the following values within the type flag:
...
```
 ...
 ```xml
  ...
  The **parent** is the seperate .ytd that you are trying to import.
  The **child** is the .ytd that will have the other textures imported into and used within your vehicle.
