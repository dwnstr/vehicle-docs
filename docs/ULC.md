...
If you are selling or releasing a vehicle that is configured to work with ULC, and you would like to provide configuration information to the customer/user this is the recommended method and template.

Be sure to include instructions to copy the below configuration into the ``Vehicles`` table in their ULC ``config.lua``

Create a file named "ulc-config.lua with your configuration like so:
```lua
{name = 'sp20',
  steadyBurnConfig = {
      forceOn = false,
...
```
The user can paste this directly into the ``Vehicles`` table in their ``config.lua`` to easily and quickly apply your desired vehicle configuration.
